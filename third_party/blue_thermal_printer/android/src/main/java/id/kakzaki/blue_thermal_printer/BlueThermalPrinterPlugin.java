...
public class BlueThermalPrinterPlugin implements FlutterPlugin, ActivityAware,MethodCallHandler, RequestPermissionsResultListener {
  ...
  private static final int REQUEST_GET_BONDED_DEVICES_PERMISSIONS = 1451;
  private static final UUID MY_UUID = UUID.fromString("00001101-0000-1000-8000-00805F9B34FB");
  private static final Handler MAIN_HANDLER = new Handler(Looper.getMainLooper());
  private static ConnectedThread THREAD = null;
  private static ConnectedThread SCALE_THREAD = null;
  private BluetoothAdapter mBluetoothAdapter;

  private Result pendingResult;

  private EventSink readSink;
  private EventSink scaleReadSink;
  ...
  private void setup(
    ...
    synchronized (initializationLock) {
      ...
      EventChannel scaleReadChannel = new EventChannel(messenger, NAMESPACE + "/scale_read");
      scaleReadChannel.setStreamHandler(scaleReadResultsHandler);
  ...
  public void onMethodCall(@NonNull MethodCall call, @NonNull Result rawResult) {
    ...
    switch (call.method) {
      ...
      case "isScaleConnected":
        result.success(SCALE_THREAD != null);
        break;
        ...
        try {
          ...
          if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            ...
            if (ContextCompat.checkSelfPermission(activity,
              ...
              }, REQUEST_GET_BONDED_DEVICES_PERMISSIONS);
            ...
            if (ContextCompat.checkSelfPermission(activity,
              ...
              ActivityCompat.requestPermissions(activity,
                new String[] { Manifest.permission.ACCESS_COARSE_LOCATION,Manifest.permission.ACCESS_FINE_LOCATION }, REQUEST_GET_BONDED_DEVICES_PERMISSIONS);
      ...
      case "disconnectScale":
        disconnectScale(result);
        break;
  ...
  public boolean onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
    ...
    if (requestCode == REQUEST_GET_BONDED_DEVICES_PERMISSIONS) {
      ...
      if (granted) {
        ...
        pendingResult.error("no_permissions", "this plugin requires bluetooth permissions to read paired devices", null);
      }
  ...
  private void connect(Result result, String address) {
    ...
    connect(result, address, false);
  }

  private void connectScale(Result result, String address) {

    connect(result, address, true);
  }

  private void connect(Result result, String address, boolean scale) {
  ...
  private void disconnectScale(Result result) {

    disconnect(result, true);
  }

  private void disconnect(Result result, boolean scale) {
  ...
  private synchronized void clearThread(ConnectedThread thread) {
    if (thread == null) {
      return;
    }
    if (thread == THREAD) {
    ...
    if (thread == SCALE_THREAD) {
      SCALE_THREAD = null;
    }
  }

  private void emitSinkSuccess(EventSink sink, Object value) {
    if (sink == null) {
      return;
    }
    MAIN_HANDLER.post(() -> sink.success(value));
  }
  ...
  private class ConnectedThread extends Thread {
    ...
    ConnectedThread(BluetoothSocket socket, boolean scale) {
      mmSocket = socket;
      this.scale = scale;
    ...
    public void run() {
      ...
      while (true) {
        try {
          ...
          EventSink sink = scale ? scaleReadSink : readSink;
          if (sink != null && bytes > 0) {
            emitSinkSuccess(sink, new String(buffer, 0, bytes));
          }
  ...
  private final StreamHandler stateStreamHandler = new StreamHandler() {
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
      ...
      private void clearMatchingThread(BluetoothDevice device) {
        if (device == null) {
          return;
        }

        String address = device.getAddress();
        if (THREAD != null && address.equals(THREAD.getAddress())) {
          THREAD = null;
        }
        if (SCALE_THREAD != null && address.equals(SCALE_THREAD.getAddress())) {
          SCALE_THREAD = null;
        }
      }
    };
    ...
    public void onListen(Object o, EventSink eventSink) {
      ...
      registerReceiverCompat(mReceiver, new IntentFilter(BluetoothAdapter.ACTION_STATE_CHANGED));
      registerReceiverCompat(mReceiver, new IntentFilter(BluetoothDevice.ACTION_ACL_CONNECTED));
      registerReceiverCompat(mReceiver, new IntentFilter(BluetoothDevice.ACTION_ACL_DISCONNECT_REQUESTED));
      registerReceiverCompat(mReceiver, new IntentFilter(BluetoothDevice.ACTION_ACL_DISCONNECTED));
    ...
    if (Build.VERSION.SDK_INT >= 33) {
      try {
        Context.class
            ...
            .invoke(context, receiver, filter, 4);
        return;
  ...
  private final StreamHandler scaleReadResultsHandler = new StreamHandler() {
    @Override
    public void onListen(Object o, EventSink eventSink) {
      scaleReadSink = eventSink;
    }
